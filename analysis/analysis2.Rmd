...
DisjunctiveCriticalData %>%
  group_by(condition, response,workerid) %>%
  summarize(count = n()) %>%
  group_by(condition) %>%
  mutate(proportion = count / sum(count)) %>%
  ...
  theme_bw() + 
  facet_grid(workerid~condition)
```

```{r}
DistDf <- DisjunctiveCriticalData %>% 
  select(c("workerid","condition","response")) %>%
  filter(response != '[\'am\']') %>%
  mutate(
  ...
  summarize_all(mean) %>% 
  pivot_wider(names_from = condition, values_from = starts_with("is_"))
