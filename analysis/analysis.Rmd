...
```{r}
ggsave("figures/CollapsedResults.png",height=8,width=14)
```
...
```{r}
DisjunctiveParticipantCondless <- DisjunctiveData %>% 
  filter(dataType == "Critical") %>% 
  mutate(response_binary = ifelse(response == "['are']",1,0)) %>% 
  group_by(workerid) %>% 
  summarize(
...
```{r}
DisjunctiveParticipant %>% 
  ggplot(aes(x=areRate)) + 
  geom_bar() + 
  labs(x="Rate of 'are' production",y="# of Participants") + 
  facet_wrap(~cond)
```
