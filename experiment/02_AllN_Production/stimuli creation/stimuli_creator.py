...
class sentence: 
    def __init__(self, id, conj1, conj2, predicate, text, conj1_agr, conj2_agr, dataType, cond):
        self.id = id
        self.conj1 = conj1
        self.conj2 = conj2
        self.predicate = predicate
        self.text = text
        self.conj1_agr = conj1_agr
        self.conj2_agr = conj2_agr
...
for i in pronouns:
    for i2 in pronouns:
        if i.form != i2.form:
            allCombinations.append((i, i2))
        elif i.form in ["I","we"]:
            pass
        else:
            allCombinations.append((i, i2))

for touple in allCombinations:
    print(touple[0].agr+" or "+touple[1].agr)

for selection1, selection2 in allCombinations:
    for i in range(5):
        selection1 = selection1
        selection2 = selection2
        if selection1.form == "it":
            conj1 = "the "+singletons.pop()
            conj1_agr = selection1.agr
...
plurals2 = ["sorcerers","actresses","fishers","programmers","attendants","dog trainers","graduates","candlemakers","shoemakers","workers","employees", "children","Martians","ghosts","mummies","lawyers", "wolves", "cooks", "performers", "acrobats", "astrologists", "bartenders", "employers", "butlers", "cashiers", "cartoonists", "clairvoyants", "diplomats", "florists", "humanitarians", "interpreters", "investors", "janitors", "jugglers", "knitters", "lifeguards", "manicurists", "millionaires", "novelists", "nutritionists", "orthodontists", "pharmacists", "pianists", "professors", "reporters", "runners", "sculptors", "screenwriters", "shoppers", "sunbathers", "tailors", "wrestlers", "zoologists", "taxidermists", "tattoo artists", "commanders", "interlopers", "zombies", "pedestrians", "palm readers", "jewelers", "informants", "hitchhikers", "gymnasts", "guards", "hairdressers", "fortune tellers", "realtors", "environmentalists", "detectives", "customers", "archaeologists", "newsboys", "stunt doubles", "villains", "graphic designers", "gameshow hosts", "museum curators", "technical writers", "security specialists", "bodyguards", "wedding planners", "maids"]
...
while len(areTexts) <= 19:
    ...
    if Conj1CoinFlip == "spine":
        ...
        conj2_agr = "3P"
    if Conj2CoinFlip == "heads":
        conj2 = "I"
        conj2_agr = "1S"
    if Conj2CoinFlip == "tails":
        conj2 = "we"
        conj2_agr = "1P"
    if Conj2CoinFlip == "underbelly":
        conj2 = names.pop()
        conj2_agr = "3S"
    if Conj2CoinFlip == "spine":
    ...
    stim = sentence(
        ...
        id = stimCount,
        dataType = "filler_are",
        cond = conj1_agr+"_and_"+conj2_agr
    )
    if conj1 != conj2:
        stimCount += 1
        stimList.append(stim)
        areTexts.append(stim)
    else:
